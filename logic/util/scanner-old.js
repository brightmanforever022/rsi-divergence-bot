...
const searchForFailedDivergences = (rsiData, priceData) => {
    ...
    for(let i = Object.keys(rsiData).length - 1; i >= 0; i--) {
        ...
        if(searchingForHiddenBullish && rsi < lowestRSIHiddenBullish) {
            //console.log(Object.keys(priceData)[i])
            searchingForHiddenBullish = false;
...
//    console.log("Alert exchange " + exchange);
    ...
    console.log("updating stocks.....................", JSON.stringify(currentAlerts))
...
const hourlyUpdate = async (tickers, filteredTickers60min, filteredTickers4h, filteredTickers1d,  filteredTickers1w, filteredTickers1m) => {
    ...
    if((hour >= 15 && hour <= 20 && minute === 31) || (hour === 23 && minute === 10)) {
        console.log("Updating");
        let latestDateOfFirstElement;
        for(let i = 0; i < tickers.length; i++) {
            console.log(tickers[i] + " " + i + "/" + tickers.length + " " + (new Date()).toString());
	    console.time("Updating");
            if(!fs.existsSync("./historicalData/60min/" + tickers[i] + ".json"))
                continue;
           
            
	        let result, result1w, result1m;
            if (hour === 21) {
		...
		result = await updateQuotesHandler(tickers[i], "60min", 2);
                result1w = await updateQuotesHandler(tickers[i], "1w", 2);
                result1m = await updateQuotesHandler(tickers[i], "1m", 2);
            }
            else
                result = await updateQuotesHandler(tickers[i], "60min", 1);
            if(!result)
                continue;
            if(i === 0) {
                latestDateOfFirstElement = Object.keys(result.rsiData)[0];
            }
	    
            if(Object.keys(result.rsiData)[0] !== latestDateOfFirstElement) {
                result = await updateQuotesHandler(tickers[i], "60min", 2, true);
            }
            if(filteredTickers60min.includes(tickers[i])) {
                await checkForDivergences(tickers[i], result, "60min");
                console.log("Checked for divergences");
            }
            if((hour === 18 && minute === 31) || (hour === 21 && minute === 1)) {
                 //console.time("Checking for 4h");
                let isEOD = false;
                if(hour === 0) {
                    isEOD = true;
                }
		console.log("4h");
                if(filteredTickers4h.includes(tickers[i])) {
                    const higherTimeframeResult = getHigherTimeframe(result.priceData, "4h", isEOD);
                    if(!higherTimeframeResult) {
                        continue;
                    }
                    await checkForDivergences(tickers[i], higherTimeframeResult, "4h");
                }
                   
                if(hour === 21 && minute === 1) {
                    console.log("1d");
		    if(filteredTickers1d.includes(tickers[i])) {
                        const dailyTimeframeResult = getHigherTimeframe(result.priceData, "1d");
                        if(!dailyTimeframeResult)
                            continue;
                        await checkForDivergences(tickers[i], dailyTimeframeResult, "1d");
                    }
                    console.log("1w");
                    if(filteredTickers1w.includes(tickers[i]) && result1w && Object.keys(result1w.rsiData).length > 0   && Object.keys(result1w.rsiData)[0].substring(0, 15) === "Mon Jan 11 2021") {
                        await checkForDivergences(tickers[i], result1w, "1w");
                    }
		    console.log("1m");
                    if(filteredTickers1m.includes(tickers[i]) && result1m && Object.keys(result1m.rsiData).length > 0 && Object.keys(result1m.rsiData)[0].substring(0, 15) === "Mon Jan 11 2021") {
                        await checkForDivergences(tickers[i], result1m, "1m");
                    }
...
const openSocket = (startedToday = false, stocksBars = {}, barsCreated = {}, dailyData = {}, didReset = false) => {
    ...
    ws.on("message", ( data ) => {
        ...
        data.map(( msg ) => {
            ...
            if(msg.ev === "AM") {
                ...
                if(hour === 21 && !didReset) {
			        ...
			        if(filteredTickers1D.includes(msg.sym) && result) {
                                    ...
                                    //const result1D = getHigherTimeframe(result.priceData, "1d", true);
                                    
                                    const result1D = updateDaily(Object.keys(stocksBars)[i], dailyData[Object.keys(stocksBars)[i]], Object.keys(result.priceData)[0]);
                                    //const result1D = await updateQuotesHandler(tickers[i], "1d", 2, false);
				    if(result1D)
                                        checkForDivergences(Object.keys(stocksBars)[i], result1D, "1d");
                                    /*console.log("1W");
                                    if(false) {
                                     //   const weekly = await updateQuotesHandler(tickers[i], "1w",  2, false);
                                        //const monthly = await updateQuotesHandler(tickers[i], "1m", 2, false);
                                        if(false) {
                                            checkForDivergences(Object.keys(stocksBars)[i], weekly, "1w");
                                        }
                                        if(false) {
                                            checkForDivergences(Object.keys(stocksBars)[i], monthly, "1m");
                                        }
                                    }*/
                                }
			...
			updateWeeklyAndMonthly(Object.keys(stocksBars)).then(() => {});
...
(async () => {
	...
	if(date.getDay() == 5 && date.getHours() == 22) {
            ...
            if(lastFriday() === date.getDate())
                monthly = true;
            console.log("essa");
            updateWeeklyAndMonthly(tickers, monthly).then(() => {console.log("finished");});
        }
    }, 60 * 1000 * 60);
//  await updateQuotesHandler("TSLA", "1d", 2, false);
    //const newRSI = {};
    //const newPrice = {}; 
//   const result = await updateQuotesHandler("AKER", "1d", 2, false);
    //const result60 = JSON.parse(fs.readFileSync("./historicalData/60min/PSTV.json"));
     /*for(let i = 0; i < Math.min(50, Object.keys(result.rsiData).length); i++) {
        const date = Object.keys(result.rsiData)[i];
        newRSI[date] = Object.values(result.rsiData)[i];
        newPrice[date] = Object.values(result.priceData)[i];
    ...
    const divergences = searchForFailedDivergences(newRSI, newPrice);*/
//    await checkForDivergences("PSTV", result, "1d");
 //   console.log(divergences);
  ...
  //  for(let i = 0; i < tickers.length; i++) {
    //   console.log(i);
        //console.time("Updated: " + tickers[i]);
    //    const result = await updateQuotesHandler(tickers[i], "1m", 2, false);
       // await checkForDivergences(tickers[i], result, "1w");
        //const result2 = await updateQuotesHandler(tickers[i], "60min", 10, false);*/
     //   await saveHistoricalData(tickers[i], "1d");*/
        //const result2 = await updateQuotesHandler(tickers[i], "1d", 10, false);
        /*if(!result || !result.priceData)
             continue;
        const result4h = getHigherTimeframe(result.priceData,"4h", true);
        if(result4h && result4h.priceData) {
             await checkForDivergences(tickers[i], result4h, "4h");
        }*/
        //const result1d = getHigherTimeframe(result.priceData,"1d", true);
        /*if(result1d && result1d.priceData) {
             await checkForDivergences(tickers[i], result1d, "1d");
        }

        const result1w = await updateQuotesHandler(tickers[i], "60min", 2, false);	
        if(result1w && result1w.priceData) {
             await checkForDivergences(tickers[i], result1w, "1w");
        }*/
/*	const data = await saveHistoricalData(tickers[i], "1d");
        await checkForDivergences(tickers[i], data, "1d");*/
//    }
  
    //const result = await updateQuotesHandler("WTTR", "60min", 10, false);
    /*
    */
    //await getAllTickers();*/
    //await saveHistoricalData("TSLA", "60min");

    //updateQuotesFile("TSLA", {"date":"Jan 15 2021 21:00:00 GMT+0000 (Greenwich Mean Time)","open":825,"close":826.16,"high":830.49,"low":822.32}, "60min");
})();
