...
bot.onText(RegExp("/load 4h"), (msg, match) => {
  ...
  bot.onText(RegExp("/load 3m"), (msg, match) => {
    chatId["3m"] = msg.chat.id;
    bot.sendMessage(chatId["3m"], "Will send alerts for 3m timeframe.");
  });
...
const getData = async (stock, timeframe, updating3m = false) => {
    const requestPromise = new Promise((resolve, reject) => {
        ...
        const req = https.get({
            ...
            response.on("end", () => {
                const a = result;
      		result = result.replace("\r", "").replace("END", "");
		let error = false;
		result = result.split("\n");
    ...
    if(timeframe === "3m") {
        let opens = Object.values(priceData).map(x => x.Open);
        let highs = Object.values(priceData).map(x => x.High);
        let lows = Object.values(priceData).map(x => x.Low);
        let closes = Object.values(priceData).map(x => x.Close);
	...
	dates.reverse();
	let longerData = {};
	if(fs.existsSync("./historicalData/3m/" + stock + ".json")) {
	    if(updating3m) {
		let temp = JSON.parse(fs.readFileSync("./historicalData/3m/" + stock + ".json"));
		for(let i = Object.keys(temp.priceData).length - 1; i >= 0; i--) {
		    longerData[Object.keys(temp.priceData)[i]] = Object.values(temp.priceData)[i];
		}
	    }
	}
	let openedBar = false;
	for(let i = 0; i< dates.length; i++) {
	    const month = dates[i].substring(4, 7);
	    ...
	    if(month === "Feb" || month === "May" || month === "Aug" || month === "Nov") {
                if(longerData[dates[i - 1]] && openedBar) {
		    longerData[dates[i - 1]].Close = closes[i];
		    longerData[dates[i - 1]].High = Math.max(longerData[dates[i - 1]].High, highs[i]);
		    longerData[dates[i - 1]].Low = Math.min(longerData[dates[i - 1]].Low, lows[i]);

		}
            }
	    if(month === "Mar" || month === "Jun" || month === "Sep" || month === "Dec") {
                if(longerData[dates[i - 2]] && openedBar) {
		    openedBar = false;
                    longerData[dates[i - 2]].Close = closes[i];
                    longerData[dates[i - 2]].High = Math.max(longerData[dates[i - 2]].High, highs[i]);
                    longerData[dates[i - 2]].Low = Math.min(longerData[dates[i - 2]].Low, lows[i]);
	    ...
	    //if(dates[i] )
         ...
         const prices = Object.values(longerData).map(x => x.Close);
         priceData = longerData;
         rsiData = RSICalculator.calculate({
             values: prices,
             period: 14
         });
	priceDataKeys = Object.keys(longerData);
        priceDataKeys.reverse();
    ...
    for(let i = 0; i < rsiData.length; i++) {
        const key = priceDataKeys[i];
        rsiMapped[key] = rsiData[i];
        priceMapped[key] = priceData[key];
    }
...
const saveHistoricalData = async (ticker, timeframe, update3m) => {
    
    let data = await getData(ticker, timeframe, update3m);
    if(data === "403") {
        //console.log("403");
        const waitForRateReset = new Promise((resolve, reject) => {
            const interval = setInterval(() => {
                //console.log("Retrying...");
                getData(ticker, timeframe, update3m).then(e => {
                    if(e !== "403") {
    ...
    fs.writeFileSync("./historicalData/" + timeframe + "/" + ticker + ".json", JSON.stringify(data));
    return data;
};
...
const sendAlert = async (ticker, type, timeframe, date, price, averageVolume) => {
    console.log("Alert " + ticker + " " + type + " " + timeframe + " " + date + " " + price + " " + averageVolume);
    /*const requestPromise = new Promise((resolve, reject) => {
        const req = https.get({
            host: "api.tradestation.com",
    ...
    let sharesFloat = 0;
    try {
        // get float data from Alphavantage due to ticker
        const request = await fetch(
            `https://www.alphavantage.co/query?function=OVERVIEW&symbol=${ticker}&apikey=${ALPA_API_KEY}`
          );
...
const checkForDivergences = (ticker, result, timeframe) => {
    ...
    const volumeData = JSON.parse(fs.readFileSync("./historicalData/1d/" + ticker + ".json")).priceData;
    let averageVolume = 0;
    let h = 0;
    for(let i = 0; i < Math.min(10, Object.keys(volumeData).length); i++) {
        if(volumeData[Object.keys(volumeData)[i]].Volume) {
            h++;
	    averageVolume += volumeData[Object.keys(volumeData)[i]].Volume;
        }
    }
    averageVolume = averageVolume / h;
    for(let y = 0; y < divergences.bullish.length; y++) {
      ...
      //          fs.appendFileSync("./" + timeframe + ".txt", ticker + " bullish \n"); 
                sendAlert(ticker, "bullish", timeframe, Object.keys(newPrice)[0], newPrice[Object.keys(newPrice)[0]]["Close"], averageVolume);
        }
    } 
    for(let y = 0; y < divergences.bearish.length; y++) {
        if(divergences.bearish[y].end === currentDate) {
             //fs.appendFileSync("./" + timeframe + ".txt", ticker + " bearish \n");
                ...
                sendAlert(ticker, "bearish", timeframe, Object.keys(newPrice)[0], newPrice[Object.keys(newPrice)[0]]["Close"], averageVolume);
             }
    } 
};
    ...
    ws.on("message", ( data ) => {
        ...
        data.map(( msg ) => {
            ...
            if(msg.ev === "AM") {
                ...
                const usDate = date.toLocaleString("en-US", {timeZone: "America/New_York", hour12: false}).split(", ")[1].split(":");
                const hour = parseInt(usDate[0]);
                const minute = parseInt(usDate[1]);
                ...
                if((hour === 9 && minute >= 30) || (hour >= 10 && hour <= 15)) {
                    let hourIndex;
		    if(hour === 9) {
		        marketOpenHour = date.getHours();	
		    }
		    else if(hour == 10) {
			marketOpenHour = date.getHours() - 1;
		    }
                    didReset = false;
		            let check4H = false;
                    if((hour === 9 && minute >= 30) || (hour === 10 && minute <= 29))
                        hourIndex = 0;
                    else if((hour === 10 && minute >= 30) || (hour === 11 && minute <= 29))
                        hourIndex = 1;
                    else if((hour === 11 && minute >= 30) || (hour === 12 && minute <= 29))
                        hourIndex = 2;
                    else if((hour === 12 && minute >= 30) || (hour === 13 && minute <= 29))
                        hourIndex = 3;
                    else if((hour === 13 && minute >= 30) || (hour === 14 && minute <= 29)) {
                        hourIndex = 4;
 		                check4H = true;
		            }
                    else if((hour === 14 && minute >= 30) || (hour === 15 && minute <= 29))
                        hourIndex = 5;
   				...
   				if(!dailyData[msg.sym].Volume) {
				    dailyData[msg.sym].Volume = stocksBars[msg.sym][stocksBars[msg.sym].length - 1].volume;
				}
				else {
				     dailyData[msg.sym].Volume += stocksBars[msg.sym][stocksBars[msg.sym].length - 1].volume;
				}
                        ...
                        modifiedDate.setUTCHours(marketOpenHour + hourIndex, 30);
                        if(hourIndex === 6) {
			                modifiedDate.setUTCHours(marketOpenHour + 6, 0);
                        }
			...
			if(!stocksBars[msg.sym][lastIndex].volume) {
			    stocksBars[msg.sym][lastIndex].volume = msg.v;
			}
			else {
                            stocksBars[msg.sym][lastIndex].volume += msg.v;
			}
...
const updateWeeklyAndMonthly = async (tickers, endOfMonth, date) => {
	...
	if(endOfMonth) {
		...
		let result3m = await saveHistoricalData(tickers[i], "3m", true);
		//console.log("checking " + tickers[i] + " " + i);   
		await checkForDivergences(tickers[i], result3m, "3m");
...
(async () => {
    ...
    const tickers = getTickersList();
    setInterval(() => {
        getAccessToken().then(x => TOKEN = x);
    }, 15 * 1000 * 60);*/
    //await updateWeeklyAndMonthly(tickers, true, new Date());
//    console.log(tickers.length)
  //  for(let i = 0; i < tickers.length; i++) {
        //await updateQuotesHandler(tickers[i], "60min", 50, false);
//	await saveHistoricalData(tickers[i], "1d", false);
//	await saveHistoricalData(tickers[i], "1w", false);
	//await saveHistoricalData(tickers[i], "3m", false); 
//	await updateQuotesHandler(tickers[i], "1d", 3, false);
//	await updateQuotesHandler(tickers[i], "1w", 3,  false);
	//if(fs.existsSync("./historicalData/1m/" + tickers[i] + ".json")) {
	  //  const data = JSON.parse(fs.readFileSync("./historicalData/1m/" + tickers[i] + ".json"));
	   // delete data.rsiData[Object.keys(data.priceData)[0]];
    	   // delete data.priceData[Object.keys(data.priceData)[0]];
	   // await checkForDivergences(tickers[i], data, "1m");

	    
	//}
/*	if(fs.existsSync("./historicalData/1w/" + tickers[i] + ".json")) {
            const data = JSON.parse(fs.readFileSync("./historicalData/1w/" + tickers[i] + ".json"));
            delete data.rsiData[Object.keys(data.priceData)[0]];
            delete data.priceData[Object.keys(data.priceData)[0]];
            await checkForDivergences(tickers[i], data, "1w");
...
//     console.log(i); 
  //  }
    ...
    console.log(new Date().toLocaleString("en-US", {timeZone: "America/New_York", hour12: false}));
	...
	const tickers = getTickersList();
	    ...
	    isLastDayOfMonth = test.getDate() === 1;
	    if(lastFriday() === date.getDate() || isLastDayOfMonth)
                monthly = true;
            updateWeeklyAndMonthly(tickers, monthly, date).then(() => {console.log("Finished 1M and 1W");});
        }
        if(date.getHours() === 2) {
   ...
   openSocket();
//    await saveHistoricalData("KO", "3m", true);
